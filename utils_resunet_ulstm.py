...
def retrieve_idx_percentage(reference, patches_idx_set,patch_size, pertentage):
    count = 0
    new_idx_patches = []
...
def matrics_AA_recall(thresholds_, prob_map, ref_reconstructed, mask_amazon_ts_, px_area):
    ...
    for thr in thresholds:
        ...
        mm = np.hstack((recall_, precision_, f1, aa))
...
def cov_layer(x, filter_size, kernel_size=3): 
    x = TimeDistributed(Conv2D(filter_size, kernel_size, activation='relu', padding='same'))(x)
    return x

def mpool_layer(x):
    maxpool1 = MaxPool2D((2 , 2))
    x = tf.keras.layers.TimeDistributed(maxpool1)(x)
    return x

def tconv_layer(x,filter_size, kernel_size=3, strides=(2,2)):
    x = Conv2DTranspose(filter_size, kernel_size, strides=strides, activation='relu', padding='same')(x)
    return x	

def up_conv_layer(x,filter_size, kernel_size=3, strides=(2,2)):
    x = Conv2D(filter_size, kernel_size, activation = 'relu', padding = 'same')(UpSampling2D(size = strides)(x))
    return x	

def U_ConvLSTM(n_time, rows, cols, channels, num_classes, f1):
    inputs  = Input(shape=(n_time, rows, cols, channels), name='input')

    conv1 = cov_layer(inputs, f1)
    pool1 = mpool_layer(conv1)
  
    convLSTM1 = ConvLSTM2D(filters = f1, kernel_size = (3, 3), activation='relu',
                           data_format='channels_last',
                           padding = 'same', return_sequences = True)(pool1)

    conv2 = cov_layer(convLSTM1, f1*2)
    pool2 = mpool_layer(conv2)

    convLSTM2 = ConvLSTM2D(filters = f1*2, kernel_size = (3, 3), activation='relu',
                           data_format='channels_last',
                           padding = 'same', return_sequences = True)(pool2)

    conv3 = cov_layer(convLSTM2, f1*4)
    pool3 = mpool_layer(conv3)

    convLSTM3 = ConvLSTM2D(filters = f1*4, kernel_size = (3, 3), activation='relu',
                           data_format='channels_last',
                           padding = 'same', return_sequences = False)(pool3)
    
    tconv3 = tconv_layer(convLSTM3,f1*4)
    merge3 = concatenate([convLSTM2[:,-1], tconv3], axis=-1) 

    tconv2 = tconv_layer(merge3,f1*2)
    merge2 = concatenate([convLSTM1[:,-1], tconv2], axis=-1)

    tconv1 = tconv_layer(merge2,f1) 
    
    out = Conv2D(filters = num_classes, kernel_size = (1, 1),
                 activation = 'softmax',
                 padding = 'same', data_format = 'channels_last')(tconv1)

    return Model(inputs = inputs, outputs = out)


def color_map(prob_map, ref_reconstructed, mask_no_considered, th):
    ...
    #output_map[clipping_mask_ == 0] = 0    # no-deforestation (true negatives) =  darkblue
    output_map[true_positives == 1] = 1     #deforestation (true positives) = darkred
    output_map[diff_image == 1] = 2         #false positives = yellow
    output_map[diff_image == -1] = 3        #false negatives = cyan
    output_map[mask_no_considered == 2] = 4 #past deforestation = darkgrey
